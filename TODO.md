...
- [ ] **The restore cooldown's base window does not reach the measured restore-latency tail, and
      ...
      **p90 129.7 s,
      ...
      distribution, but it is keyed on instability, not on restore latency, so it does not arm for
      this.
      ...
      *The stamp.* `recordRestoreActuation` (`lib/executor/planExecutor.ts`) sets both the
      ...
      `restore/steppedRestoreGates.ts` ~67). One device retrying on a cadence therefore delays other
      devices' resumes. The 2.17.5 stepped binary-restore gate
      (`hasStableSteppedLoadBinaryRestoreActuation`) is rate-limited per device specifically to keep
      its worst case at the same cadence as an ordinary restore rather than per rebuild cycle, but
      the underlying global/per-device mismatch is still there. The candidate fixes are to make the
      restore cooldown per-device throughout, or to scope the global stamp to the device that
      actuated.

      *Why one ruling.* The global stamp is the only cross-device pacing there is, and the
      distribution above argues 60 s of it is already too short. Scoping the stamp per-device
      removes that pacing outright, so this exposure becomes unbounded unless a per-device stamp
      ships with a carve-out preserving some cross-device gate. Decide the keying and the size
      together.

      Done when the cooldown that gates a second restore is sized from this distribution and keyed
      on a ruled-on stamp scope, with the sizing argument written down and an SDK-boundary e2e that
      fails at the old window. Measurements from 2026-08-13; re-scoped onto the cooldown 2026-08-28.
...
- [ ] **A shed device waiting out a restore gate can carry no gate reason, so it defaults to
      ...
      way.**
      *Persona:* owner (`notes/personas.md`) opening device detail on a device that is held back, to
      find out what is holding it.

      `buildBaseReason` (`lib/plan/planReasons.ts` ~42) ends with `?? { code: capacity }` for a
      device that is planned `shed`, was not shed THIS cycle, and whose own reason normalized away.
      That default is required — `SHED_REASON_RULES` (`lib/plan/planReasonsValidation.ts`) is an
      allow-list of limit-shaped codes and a shed device must carry one — but it fires in windows
      where no limit is binding at all. Prod 2026-07-25: a charger shed during a 10 s overshoot read
      `capacity` on 5 cycles while the house sat at 1.4 kW against a 5.4 kW pace; it was really
      waiting out `cooldown_shedding` → `meter_settling` → `restore_need`, all of which DO set their
      own reasons on the cycles they cover. So the real defect is the gap: the restore path does not
      always attach a gate reason, and the default then speaks for it. Fix is to close that gap
      (audit which restore-gate paths leave the reason unset), NOT to loosen the default to `none`
      — an attempt to return `none` when nothing binds broke 6 integration tests precisely because
      it violated the allow-list.

      *The startup-reservation hold is the same gap with a second, named honest answer.* Both routes
      into it now COUNT toward the held-back clock — the 2026-08-08 ruling is that the clock runs
      whenever PELS turned the device off, and a reservation is PELS holding it for a
      higher-priority device. What still differs is the attributed cause. When the restore/hold lane
      evaluated the device it sets the `reservedForStart` code and the cause reads
      `reserved_for_start` ("Waiting so X can start"). When it did not (fresh shed, over-pace,
      cooldown), `finalizeCeilingReason` leaves the ceiling code in place and the cause reads
      `capacity` / `insufficient_headroom` ("Waiting for available power") — which tells the owner
      to free power for a hold no amount of freed power lifts.

      *Card versus diagnostics.* The card no longer *says* "Limited by the hard cap" for the unknown
      case (the held fallback was split off from `PLAN_STATE_CAPACITY_STATUS` in the same PR), and
      the card copy for both reservation routes was unified 2026-08-07 (the holder's name rides
      along as a display field), so the CARD is honest either way. Device-detail diagnostics still
      are not — and that is where an owner goes when the card is not enough. A genuine `capacity`
      reason on a device no limit is holding is still wrong there. (Softened 2026-08-02:
      `finalizeCeilingReason` now attaches the per-cycle admission gap to that default — when
      nothing binds, admission passes and the card shows the bare waiting line rather than a phantom
      number — but the wrong CODE still reaches diagnostics/starvation classification.)

      *Why the reservation half was not fixed with the copy — the guard any fix must respect:* the
      honest code, `reservedForStart`, is in `RESTORE_ADMISSION_HOLD_REASON_CODES` and builds no
      actuation intent. Minting it at reason normalization would cancel an in-flight shed command's
      retry while the device keeps drawing inside the reserved block —
      `test/integration/planHeadroomReserve.test.ts` § "keeps asserting the floor while the shed has
      not materialized" is the guard.

      *Fix direction:* thread an `observedAtShedFloor`-equivalent into `finalizeCeilingReason` (the
      setpoint lane grew one in `planReasonsHoldDecisions.ts` § `isObservedAtShedFloor` on
      2026-08-08, so binary and stepped devices still need theirs) and apply the same guarded
      rewrite the restore lane already does.

      *Scope note:* `HEADROOM_RESERVE_MAX_MS` is 15 minutes and the held-back entry delay is also 15
      minutes, so a maximally long reservation now sits exactly on the boundary — worth confirming
      against prod telemetry before tuning either constant.

      Source: prod investigation + test suite, 2026-07-25; bare "Waiting to resume" path audit,
...
- [ ] **Bound the startup reservation on time spent HOLDING, not wall-clock since first sighting.**
      ...
      preemptive-power-reservation, 2026-08-01.
      **Includes the re-arm half.** A reservation that lapses mid-window can never re-arm even if
      ...
      bucket. Done when both the bound and the re-arm are covered. *P2*
...
- [ ] **The reachability confirmation timer and the retry timer share one key, so whichever is
      ...
      distinguishable. P2. Source: pels-runtime-reality on PR #2290, 2026-09-03. [P2]
...
- [ ] **Smart-task permission presentation: the read-only granted-permissions row sits in "What
      PELS has learned".** On the task detail, the granted-permissions row renders inside the
      learned-inputs card; a permission is a setting, not something learned — move it beside the
      task inputs. Source: 2026-08-02 release review, pels-ux-fit. [P2]
...
- [ ] **Smart-task diagnostics: distinguish an active meter source from a missing device before its deadline.**
      ...
      *Hypothesis:* the source-filtered lifecycle device set omits the meter, so the still-enabled task
      ...
      ends. Source: runtime-reality review of PR #1873, 2026-07-23.
      **Two new arms are required, not one.** A meter-source exclusion and an automatic
      ...
      toggle. Done when both arms and both mappings exist. [P2]
...
- [ ] **Smart-task preview keeps a missing committed task reserved after the live grace expires.**
      *Persona:* owner previewing a lower-priority task after a higher task's device has stayed
      ...
      then add a post-grace preview-parity regression. Source: pels-runtime-reality review of
      the relative-priority PR, 2026-08-08. [P2]
...
- [ ] **Disambiguate the smart-task schedule chart's two kinds of dimmed bar.** With the
      ...
      pels-m3-critic + pels-ux-fit on the data-viz palette PR, 2026-07-02.
      Absorbed from the merged reason-clause item: an in-window, affordable hour that simply was not
      needed still has no stated why, which the same producer-resolved clause would answer. [P2]
...
- [ ] **The create widget's `preserve` write drops standing permissions it didn't name — and it
      never offered the third toggle in the first place.**
      *Persona:* any user who granted a permission by Flow and later re-creates the task from the
      **New smart task** widget with a different toggle on; Orchestrator setting permissions at
      creation time.
      ...
      *The missing toggle — same widget, one change:* the `pauseLowerPriorityDevices` rescue
      permission ships with a Flow entry (`allow_smart_task_rescue`) and full runtime behaviour
      (`lib/plan/admission/headroomReserve.ts` — it reserves power so the task can start sooner), but
      the create-smart-task widget (`widgets/create_smart_task/src/public/render.ts` + `src/api.ts`)
      still offers only the budget + limit toggles, so it can't be set at creation time from the
      widget. Add the third (ungated) toggle for parity, mapping to `'always'` like the others. Copy
      must not say anything is turned off — see `notes/ui-terminology.md`.
      ...
      *If fixed:* thread the device's standing rescue into `widgets/create_smart_task/src/api.ts`'s
      ...
      regenerates the widget bundle. The write and the toggle are the same widget — do them
      together. Files: `widgets/create_smart_task/src/api.ts`,
      `widgets/create_smart_task/src/public/render.ts`,
      `lib/objectives/deferredObjectives/objectiveWrite.ts`. Needs pels-ux-fit + pels-m3-critic + a
      Playwright screenshot pass (mobile 480 px). Source: pels-layering-guardian on editable
      smart-task permissions, 2026-08-02; toggle half from the pause-lower-priority feature,
      2026-07-01.
      **Absorbed from the merged create-screen item, whose strength rules must survive.** The
      ...
      design can be reintroduced. The reserve-power toggle the settings UI offers is also missing. *P2*
...
- [ ] **"Clear device settings" leaves the device's smart task behind, now visible as a paused card.**
      *Persona:* owner who removes a device from Homey and then purges its PELS settings. *Hypothesis:*
      ...
      `deferred_objective.<id>` key and no card on the Smart tasks list. Source: correctness lens
      ...
      **Enumerate AND purge `respect_external_off_devices`.** Adding it to the enumeration only makes
      ...
      `clearMultipleDeviceSettings` with the enumeration. [P3]
...
- [ ] **A rescue on the lowest-priority device can grant nothing and still report success.**
      ...
      *Why it's needed:* a money-action confirm that provably cannot help is the one thing the rescue
      ...
      fall through to `rescueDoneQueued` framing. Source: adversarial review, 2026-08-04. [P3]
...
- [ ] **Create Smart tasks directly from the Smart tasks page.**
      ...
      *Why it's needed:* creation is an ordinary Smart-task operation and should not require
      ...
      with or adapted from `widgets/create_smart_task/`. [P3]
...
- [ ] **A silent meter AREA has no staleness indication — the no-readings banner speaks only for
      ...
      `getPowerReadModel` in `packages/settings-ui/src/ui/power.ts`) is the one staleness surface
      after the freshness chip's removal, and before that removal a silent area still surfaced
      through the scoped hero's chip. Decide whose scope the global banner speaks for: either feed it
      from the ACTIVE home scope's tracker timestamp (scoped read carries it), or add a per-area arm
      to the banner copy. Done when silencing an area's meter for >60 s shows a user-visible
      indication while Main stays fresh, pinned by a UI test. Sharper since 2026-09-02: past the shed
      timeout the scoped hero renders NOTHING for an unmeasured cycle (`PlanHero.tsx`, owner ruling
      — no computed figure is drawn), so a silent area now shows only `Limited` cards under no
      banner and no hero; the banner arm is the only surface left to carry that state.

      **A second arm is required, not a variant: a sub-home meter area under `power_source = flow`
      renders nothing, forever.** `routeMeterReadings`
      ...
      dry-run when the source is unauthorized), so nothing unsafe happens; the gap is that the
      per-home Limits/Overview surface has nothing to render and says nothing about why. Note this
      contradicts the intent recorded at `setup/homeRuntime/homeCapacityBundleApi.ts` ("the
      ready-edge is DECOUPLED from sample arrival ... so it works in flow mode") — correct that
      comment with the fix. Per the 2026-08-16 ruling a meterless home is unmanageable and correctly
      gets no plan, and that ruling stands: the fix is a surface that SAYS so, not a fabricated plan.
      Done when a meter area on a home running `power_source = flow` renders a line naming the power
      source as the reason it has no readings, instead of a permanently blank card.
      *Hypothesis:* an owner who adds a meter area while on the flow source sees a permanently blank
      card and reads it as PELS being broken. *Persona:* the multi-home owner mid-setup. [P2]
...
- [ ] **"Leave off until turned on again": sub-home plan paths are not driven by hold changes.**
      ...
      majority — are unaffected. [P2]

- [ ] **Area rules refuse on save instead of guiding in the editor — and a one-meter home can cycle
      between two of those refusals with no diagnosis.** The name-length cap, the reserved "Main
      home" name, the eight-area cap, and the whole-home-meter requirement are enforced at the
      `ui_homes_save` seam and surface as a toast after a round trip; only non-emptiness and
      uniqueness are pre-checked inline by `validateSubHomeDraft`. The editor should carry a
      `maxlength` on the name input, an inline error for the reserved name, a disabled "Add meter
      area" button at the cap, and an inline main-meter refusal from the `shouldPromptMainHomeMeter`
      predicate `homesSettings.ts` already evaluates for the notice. Held out of the enforcing PR
      because `SubHomeDraftError` and `HomesSettingsSection.tsx` were owned by a concurrent branch;
      the server rules were deliberately scoped to the entry being written so `validateDraftName`
      can absorb them unchanged (the two validators this shares a fix with).

      *The case the missing guidance leaves worst off:* an area needs its own meter and the Main home
      needs its own, so a home whose Homey Energy report lists a single meter cannot use meter areas
      at all. Today it discovers that by bouncing between two refusals — saving an area refuses with
...
- [ ] **A smart task and "match solar surplus" cannot combine on one device — the task wins outright.**
      ...
      *Why:* without it "charge on solar surplus" and "be charged by X" are mutually exclusive, and
      an owner who wants both must pick one. *(From the surplus-tracking train, 2026-08-26.)* [P2]
...
- [ ] **A budget-axis restore rejection is attributed as capacity pressure when
      ...
      *Hypothesis:* needs projected>measured AND the projection large enough to flip the argmin;
      ...
      pels-layering-guardian corner (b), 2026-08-02.
      **Absorbed from the merged admission item, the same flag's opposite face.** It is also a false
      ...
      lever. Deriving attribution per axis at the rejection site closes both faces. [P2]
...
- [ ] **The zone tree is refetched on every snapshot refresh — 538 Homey round-trips per 12 h for
      ...
      **When it is done:** zone fetches per 12 h are down by at least an order of magnitude, and a
      ...
      planned by the wrong home. Source: Codex P1 on PR #2252, 2026-08-31. [P2]
...
- [ ] **The observed-state label is a closed set returned as `string`, so consumers hedge against
      ...
      Source: observer cleanup sweep, 2026-09-03. [P2]
...
- [ ] **Limits & safety: converge the Main-home form onto the native `.pels-input` field language.**
      *Persona:* multi-home owner who switches the shell's home scope bar between the Main home and a meter
      ...
      pels-m3-critic review of the U3 per-home Limits surface, 2026-07-20. [P2]
...
- [ ] **Generation-guard the rescue-gate state commit in `loadStarvationRescuableDevices`.** `overviewRescueGate`
      ...
      coderabbit on #1736, 2026-06-17. [P2]
...
## Diagnostics, logging and alerts
...
- [ ] **Six setup files still hold runtime state, above the boundaries `arch:check` enforces.**
      ...
      rebuilt per sample, because it closes over the request ``, and `appSnapshotHelpers.ts`'s state
      move is part of this umbrella and is not tracked separately anywhere. Each is a component in
      the wrong directory, and each migration is the same shape: move the state **and the rules that
...
- [ ] **`plan_rebuild_completed` is absent from production for the case it was added to explain.**
      ...
      `getPlanRebuildLogLevel` (`lib/plan/planRebuildMetrics.ts`) returns `'debug'` for the
      actionChanged-only rebuild — a plan whose decisions changed but issued no commands. That is a
      pino child of the `info` root, so the line is dropped: every `plan_rebuild_completed` in a
      production log carries an info-forced `reasonCode` (`initial`, `settings:*`,
      `startup_snapshot_bootstrap`) and none carries `appliedActions:false` from the debug branch.
      The comment at that branch calls it "plan debug topic", which it is not — it is not gated,
      it is discarded. Fix: emit the actionChanged-only case through
      `getDebugEmitter('plan', 'plan')` and keep the `info` cases on the module logger, so the
...
## Closed trains and historical notes

*Prior P0 closures shipped on the v2.9 train via PRs #975, #977, #978, #980,
#982, #983; surviving follow-ups demoted to P1/P2.*

*The 2026-07-25 release-review cleanup closed the prior P0 set: it partitioned realtime reconcile
work by home, finite-gated saved meter-area limits, and corrected the Main-meter setup guide.
Remaining multi-meter work is tracked as P1/P2/P3 follow-up.*
...
(What that train left open is tracked in the theme sections.)*

*P1 targeted refactors (deferred): concrete, bounded changes to specific named surfaces — not
structural re-splits, which stay P2.*
...
split follow-ups from this batch are fixed by the solar-accounting follow-up.*
...
`notes/state-management/deferred-objective-lifecycle-carveout.md`. Its remaining follow-ups are
non-blocking.*

*Persona grouping retained from the former P3 section: smart-task failure-investigation & live UX
serves the underserved Optimiser and the Failing-scenario (acute/recovering) visitors
(`notes/personas.md`).*

*Persona grouping retained from the former P3 section: EV charging serves the Optimiser / EV
commuter (`notes/personas.md`).*
...
- [x] Update `docs/configuration.md` and `docs/technical.md` for the **When the
  temperature changes outside PELS** selector: Return to mode target, Keep the new
  temperature, and Save as current mode target. That policy adopts manual changes,
  holds the active price shift until the price level changes, keeps later price
  deltas available, and leaves solar adjustments off while power limiting remains
  available. Existing disable settings retain their meaning. See
