...
export class DeviceManager {
  ...
  async setCapability(deviceId: string, capabilityId: string, value: unknown): Promise<void> {
    ...
    await setCapabilityValue({ deviceId, capabilityId, value });
  ...
  private parseDeviceList(list: HomeyDeviceLike[]): TargetDeviceSnapshot[] {
    const now = Date.now();
    return list.map((device) => this.parseDevice(device, now)).filter(Boolean) as TargetDeviceSnapshot[];
  }
  ...
  private parseDevice(device: HomeyDeviceLike, now: number): TargetDeviceSnapshot | null {
    ...
    const powerEstimate = this.updateAndGetPowerEstimate({ device, deviceId, deviceLabel, powerRaw, now });
  ...
  private getDeviceClass(device: HomeyDeviceLike): string | null {
    if (typeof device.class === 'string' && device.class.trim()) return device.class.trim();
    return null;
  }
  ...
  private resolveDeviceCapabilities(capabilities: string[]): { targetCaps: string[] } | null {
    if (!capabilities.includes('measure_power')) return null;
    const targetCaps = this.getTargetCaps(capabilities);
    const hasOnOff = capabilities.includes('onoff');
    if (targetCaps.length > 0 && !capabilities.includes('measure_temperature')) return null;
    if (targetCaps.length === 0 && !hasOnOff) return null;
    return { targetCaps };
  }
  ...
  private getCurrentOn(capabilityObj: Record<string, CapabilityValue>, powerRaw: unknown): boolean | undefined {
    if (typeof capabilityObj.onoff?.value === 'boolean') return capabilityObj.onoff.value;
    if (typeof powerRaw === 'number' && powerRaw > 50) return true;
    return undefined;
  }
  private updateAndGetPowerEstimate(params: {
    ...
    if (loadW !== null) {
      const measured = this.getMeasuredPowerKw({ deviceId, deviceLabel, powerRaw, now });
      const loadEstimate = this.getPowerFromLoad({ deviceId, deviceLabel, loadW, expectedOverride });
      return { ...loadEstimate, measuredPowerKw: measured.measuredPowerKw };
    }
    return this.getPowerFromMeasurement({ deviceId, deviceLabel, powerRaw, expectedOverride, now });
  }
  ...
  private getPowerFromLoad(params: {
    ...
    if (expectedOverride) {
      ...
      return { powerKw: expectedOverride.kw, expectedPowerKw: expectedOverride.kw, expectedPowerSource: 'manual', loadKw };
    }
    this.logger.debug(`Power estimate: using settings.load for ${deviceLabel}: ${loadKw.toFixed(3)} kW`);
    return { powerKw: loadKw, expectedPowerKw: loadKw, expectedPowerSource: 'load-setting', loadKw };
  }
  ...
  private getPowerFromMeasurement(params: {
    ...
    if (expectedOverride) {
      return this.resolveOverrideEstimate({ deviceLabel, expectedOverride, measuredKw: measured.measuredKw });
    }
    if (peak) {
      this.logger.debug(`Power estimate: using peak measured for ${deviceLabel}: ${peak.toFixed(3)} kW`);
      return { powerKw: peak, expectedPowerKw: peak, expectedPowerSource: 'measured-peak', measuredPowerKw: measured.measuredPowerKw };
    }
    this.logger.debug(`Power estimate: fallback 1 kW for ${deviceLabel} (no measured/override/load)`);
    return { powerKw: 1, expectedPowerKw: undefined, expectedPowerSource: 'default', measuredPowerKw: measured.measuredPowerKw };
  }
  ...
  private getMeasuredPowerKw(params: {
    ...
    if (typeof powerRaw !== 'number' || !Number.isFinite(powerRaw)) return {};
    if (powerRaw === 0) return { measuredKw: 0, measuredPowerKw: 0 };
  ...
  private resolveOverrideEstimate(params: {
    ...
    if (measuredKw !== undefined && measuredValue > expectedOverride.kw) {
      ...
      return { powerKw: measuredValue, expectedPowerKw: measuredValue, expectedPowerSource: 'measured-peak', measuredPowerKw: measuredKw };
    }
    this.logger.debug(`Power estimate: using override for ${deviceLabel}: ${expectedOverride.kw.toFixed(3)} kW`);
    return { powerKw: expectedOverride.kw, expectedPowerKw: expectedOverride.kw, expectedPowerSource: 'manual', measuredPowerKw: measuredKw };
  }
  ...
  private resolveHomeyInstance(homey: Homey.App): Homey.App['homey'] {
    if (this.isHomeyAppWrapper(homey)) return homey.homey;
  ...
  private async getRawDevices(path: string): Promise<Record<string, HomeyDeviceLike> | HomeyDeviceLike[]> {
    ...
    if (!api?.get) throw new Error('Homey API client not available');
