...
Longer term, the planner should probably decide only what to shed. Step and temperature projection describe how to shed, so `steppedDesiredStepByDeviceId` and `temperatureShedTargets` should move toward the executor or executable-plan boundary when that contract is split.
