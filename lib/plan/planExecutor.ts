...
export class PlanExecutor {
  ...
  public handleConfirmedBinaryCommand(params: {
    ...
    if (pending.desired) {
      ...
      this.recordShedActuation(deviceId, liveDevice.name, now, pending.isSwapShed);
    }
  ...
  private async applyShedOff(
    ...
    const swapReason = isSwap ? formatDeviceReason(reason) : undefined;
    return {
      handled: true,
      wrote: isSwap
        ? await this.applySheddingToDevice(dev.id, dev.name, swapReason, true)
        : await this.applySheddingToDevice(dev.id, dev.name, swapReason),
    };
  }
   ...
   */
  private async turnOffDevice(deviceId: string, name: string, reason?: string, isSwapShed = false): Promise<boolean> {
