/**
 * Observation trust gates. Each accessor resolves a single question that
 ...
 *    objectives bridge).
 *
 * Lives in `lib/utils/` so both `lib/observer/` and `lib/device/` can import
 * a single source. The helper is pure structural code over
 * `ObservationTrustInput = { observationStale?: boolean }` with no
 * observer-specific runtime state.
 */
