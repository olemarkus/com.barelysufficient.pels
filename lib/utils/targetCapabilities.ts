...
const clampTargetCapabilityValue = (
  target: Partial<Pick<TargetCapabilitySnapshot, 'min' | 'max'>> | null | undefined,
  value: number,
): number => {
...
const roundTargetCapabilityValue = (
  target: Partial<Pick<TargetCapabilitySnapshot, 'min' | 'max' | 'step'>> | null | undefined,
  value: number,
): number => {
...
export const normalizeTargetCapabilityValue = (params: {
  target?: Partial<Pick<TargetCapabilitySnapshot, 'min' | 'max' | 'step'>> | null;
  value: number;
}): number => {
