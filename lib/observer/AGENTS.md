...
- **Snapshot refresh and idle classification** still consult the `observationStale` flag for their own reasons; this rule is scoped to "consuming the current value for planning," not all observation trust. (Removing `observationStale` outright is tracked separately.)
