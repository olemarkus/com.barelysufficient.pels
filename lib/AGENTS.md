...
- Exported boundary types and module entry/hub files carry a docblock stating ownership, the invariants callers can rely on, and the governing note or doc. Pure internal helpers don't need one. House-style references: `lib/executor/executorDeviceRead.ts`, `lib/price/combinedPricesReader.ts`.
