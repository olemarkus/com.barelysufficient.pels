...
- `lib/core/DeviceManager` owns observed current state and device-specific actuation transport. Native stepped-load capabilities, stepped-load flow requests, synthetic capability reporting, and Homey write details belong behind this boundary.
...
- Avoid adding native-vs-flow stepped-load transport branches to planner or executor code. Put those choices in `DeviceManager` or a `lib/core` helper owned by it. Flow-backed binary control is still transitional in plan/executor code until that boundary is moved separately.
