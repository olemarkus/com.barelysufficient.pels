...
- `observationProducers.ts` — builds the observation producers (battery, solar, EV car-link, external temperature adjustments) as one seam; they emit facts and never issue device commands.
...
- `retainedPowerStore.ts` / `retainedPowerPersistence.ts` — the transport's last trusted power reading and `meter_power` anchor per device, kept across a restart on the userdata database and restored before the first read. A device's first parse after boot falls back on the restored reading exactly as a later parse falls back on the previous snapshot's, so a restart is not a gap in the reading. The store never deletes a row for a device absent from one save; only age (30 days unseen) prunes it.
...
- **Every device read passes the device-read contract, or is ignored** (owner ruling 2026-09-23; `transport/deviceReadContract.ts`). A full fetch or a realtime `device.update` must carry the capability list and, for every declared capability of PELS's model, a value of the model's type and the `lastUpdated` Homey observed it at (a positive, in-range time — `toCapabilityTimestampMs` is the one parser) — checked after a vendor device is converted to the model (Zaptec → `evcharger_charging(_state)`), so a converted charger's plug state must be a member of the enum like a native one's. The model is only what PELS reads for the device's role: a class `car` (which never survives parse) is checked only on what the EV car-link reads (`ev_charging_state`, `measure_battery`); state of charge only on an EV charger or home battery; the temperature capabilities (`measure_temperature`, `target_temperature`, `thermostat_mode`) only on a device declaring both the measurement and the target; and nothing on a class PELS does not admit. Drivers do declare capabilities that do not fit PELS's model of the device and never report them (Homey sends `value: null, lastUpdated: null`); those are not PELS's business. A capability PELS does read either has a value or the device is not yet admissible — Homey does not revert a set value to `null` (owner, 2026-09-23). Downstream of the contract, a value from a fetch or `device.update` is always dated by its source: nothing places one on an arrival or refresh time, and no "timestamp-less" branch exists to reintroduce. (A per-capability realtime event carries no stamp by design and is dated on arrival; it is its own, narrower read.) A non-conforming read is a no-op (the fetch's `DeviceListRead` carries it as present-but-unread, so the battery, solar and car-link producers neither drop the device nor count a miss): nothing from it is parsed, merged, tracked or logged as a change, and the device's previous entry stands (on a full read too — an ignored device is not removed, and a never-seen one waits for a conforming read). PELS never merges a partial payload with retained values; do not reintroduce "the update omitted it, keep the held one" retention. Each ignored read is said once per device as `device_read_ignored`, and `device_read_conforming_again` closes it.
