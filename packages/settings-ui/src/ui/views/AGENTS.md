...
- **All components must be written in Preact.** Use `.tsx` files. Do not add imperative DOM manipulation (`getElementById`, `textContent`, `innerHTML`, `hidden`, etc.) to this directory. If you need to read or write DOM state, lift it out to a non-Preact orchestrator (e.g. `planRedesign.ts`) and pass it in as props.
