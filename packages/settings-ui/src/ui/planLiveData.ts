...
const resolveCountdownRemainingSec = (
  ...
  if (isFiniteNumber(reason.countdownStartedAtMs) && countdownTotalSec !== undefined) {
    ...
    return Math.ceil(Math.max(0, endsAtMs - nowMs) / 1000);
  }
...
export const resolveDisplayPlanDeviceSnapshot = <Device extends TimedPlanDevice>(
  ...
  const displayReason = getDisplayReason(device.reason, snapshotGeneratedAtMs, nowMs);
  const displayCountdownTotalSec = resolveDisplayCountdownTotalSec(
    device.reason,
    displayReason,
  );
