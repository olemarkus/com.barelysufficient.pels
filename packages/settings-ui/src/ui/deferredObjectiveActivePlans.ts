...
// The sole consumer (`PlanDeviceCards` EV-state line) reads only `latest.hours`
// and `diagnosticReasonCode` via optional-chaining, so this returns the narrow
// `OverviewDeferredObjectiveActivePlans` view — value columns are deliberately
// unreachable here. Anything that is not a `{ plansByDeviceId: object }` blob
// (absent, malformed, or a transient empty SDK read) coerces to `null`, matching
// the recorder's permissive boot load.
export const coerceDeferredObjectiveActivePlans = (
  ...
  return {
    version: 1,
    plansByDeviceId: candidate.plansByDeviceId as Record<string, OverviewDeferredObjectiveActivePlan>,
  };
};
