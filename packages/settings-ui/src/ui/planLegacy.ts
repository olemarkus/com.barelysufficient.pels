...
import {
  ...
  type SettingsUiPlanPendingTargetCommand,
...
export type PlanSnapshot = Omit<SettingsUiPlanSnapshot, 'meta' | 'devices'> & {
...
const parsePlanSnapshot = (value: unknown): PlanSnapshot | null => {
...
const buildPlanPowerLine = (overview: DeviceOverviewStrings) => (
  overview.powerMsg ? createMetaLine('Power', overview.powerMsg) : null
);

const buildPlanStateLine = (overview: DeviceOverviewStrings) => createMetaLine('State', overview.stateMsg);

const buildPlanUsageLine = (overview: DeviceOverviewStrings) => createMetaLine('Usage', overview.usageMsg);
...
export const updatePlanPower = (_power: SettingsUiPowerStatus | null): void => {};
