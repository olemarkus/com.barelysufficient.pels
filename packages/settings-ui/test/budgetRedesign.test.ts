...
describe('resolveSplitComparison', () => {
  ...
  it('reads exact-budget tight days as on-budget, not 0.0 over', () => {
    // Yesterday-view comparison uses `sum(actualKWh)`. 24 × 2.5 = 60.0 lands
    // on the 60.0 budget — `tight` status must not read "0.0 over … — close.".
    const payload = buildPayload({
      actualKWh: Array.from({ length: 24 }, () => 2.5),
    });
    expect(resolveSplitComparison(payload, 'yesterday', 'tight'))
      .toBe('On your 60.0 budget — close.');
  });
});
...
describe('resolvePriceTagline', () => {
  ...
  it('renders the configured currency unit for non-NOK price schemes', () => {
    // Flow / Homey scheme with `{unit: '€', divisor: 1}` — prices are already
    // in whole units. `today` view blends `actualKWh` for elapsed buckets
    // (first 12 by default) with `plannedKWh` for the remainder; pin both to
    // 2.0 so the arithmetic is exact: 24 × 2.0 × 1.5 = 72.
    const payload = buildPayload({
      plannedKWh: Array.from({ length: 24 }, () => 2.0),
      actualKWh: Array.from({ length: 24 }, (_, i) => (i < 12 ? 2.0 : 0)),
      price: Array.from({ length: 24 }, () => 1.5),
    });
    expect(resolvePriceTagline(payload, 'today', { unit: '€', divisor: 1 }))
      .toBe(`≈\u00A072\u00A0€ at today's prices.`);
  });

  it('returns null when the cost-display unit is unknown', () => {
    // Flow / Homey scheme with no `priceUnit` resolves to `{unit: '', divisor: 1}`.
    // Suppress the tagline rather than print an unlabeled number.
    const payload = buildPayload({
      plannedKWh: Array.from({ length: 24 }, () => 2.0),
      price: Array.from({ length: 24 }, () => 1.5),
    });
    expect(resolvePriceTagline(payload, 'today', { unit: '', divisor: 1 })).toBeNull();
  });
});
