...
const makeEntry = (p: { id: string; deviceId: string; deviceName: string; outcome: Outcome; finalizedAtMs: number; startC: number; finalC: number; targetC: number }) => {
  ...
  return { id: p.id, deviceId: p.deviceId, deviceName: p.deviceName, objectiveKind: 'temperature', targetTemperatureC: p.targetC, targetPercent: null, deadlineAtMs, startedAtMs, finalizedAtMs: p.finalizedAtMs, startProgressC: p.startC, startProgressPercent: null, finalProgressC: p.finalC, finalProgressPercent: null, initialEnergyNeededKWh: 4, outcome: p.outcome, metAtMs: p.outcome === 'met' ? deadlineAtMs - HOUR : null, usedDeadlineReserve: false, observedIntervals: [{ fromMs: startedAtMs, toMs: deadlineAtMs }], discoveredFrom: 'observation', originalPlan: { hours: [{ startsAtMs: startedAtMs, plannedKWh: 1 }], energyNeededKWh: 4, planStatus: 'on_track', revisedAtMs: startedAtMs }, finalPlan: null, revisionCount: 1 };
};
