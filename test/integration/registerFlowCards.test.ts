...
describe('registerFlowCards', () => {
  ...
  it('accepts EV charger battery reports when the post-report snapshot lookup fails', async () => {
    ...
    expect(structuredInfo).toHaveBeenCalledWith(expect.objectContaining({
      ...
      chargerName: 'Zaptec Go',
      percent: 42,
      // The post-report lookup failed, so there is no snapshot to resolve a
      // level from — the log says so outright rather than implying a reading.
  ...
  it('rejects 0 W power reports when no configured off step exists', async () => {
    const { deps, actionListeners, structuredWarn } = buildDeps({
      getSnapshot: vi.fn().mockResolvedValue([
        {
          id: 'dev-1',
