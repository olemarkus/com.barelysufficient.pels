...
const buildDom = () => {
  document.body.innerHTML = `
    ...
    <section class="panel hidden" data-panel="budget">
      ...
      <div id="daily-budget-legend-controlled"></div>
      <div id="daily-budget-legend-planned-label"></div>
      <div id="daily-budget-legend-planned-swatch"></div>
...
describe('settings script', () => {
  ...
  it('updates cheap/expensive lists when price settings change', async () => {
    ...
    const dayKeys = new Set(
      combinedPrices.prices.map((entry) => getDateKeyInTimeZone(new Date(entry.startsAt), timeZone)),
    );
  ...
  it('toggles daily budget legend when breakdown is enabled', async () => {
    ...
    const controlledLegend = document.querySelector('#daily-budget-legend-controlled') as HTMLElement;

    expect(plannedLabel.textContent).toBe('Planned');
    expect(controlledLegend.hidden).toBe(true);
    expect(plannedSwatch.classList.contains('daily-budget-legend__swatch--uncontrolled')).toBe(false);

    const breakdownInput = document.querySelector('#daily-budget-breakdown') as HTMLInputElement;
    breakdownInput.checked = true;
    breakdownInput.dispatchEvent(new Event('change', { bubbles: true }));
    await new Promise((resolve) => setTimeout(resolve, 200));

    expect(plannedLabel.textContent).toBe('Uncontrolled');
    expect(controlledLegend.hidden).toBe(false);
