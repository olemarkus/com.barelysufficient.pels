...
class PelsStatusDevice extends MockDevice {
  ...
  async updateMode(mode: string): Promise<void> {
    ...
    try {
      await this.setCapabilityValue('pels_status', mode);
    } catch (error) {
      this.error('Failed to update pels status', error);
    }
  }
...
class PelsStatusDeviceFixed extends MockDevice {
  async onInit(): Promise<void> {
...
describe('PelsStatusDevice', () => {
  describe('updateShortfall - current behavior (fails for old devices)', () => {
    it('should fail to set alarm_generic when capability is missing', async () => {
      const device = new PelsStatusDevice();
      // Simulate old device that only has pels_status capability
      await device.addCapability('pels_status');
  ...
  describe('updateShortfall - fixed behavior', () => {
    it('should add alarm_generic capability on init if missing', async () => {
      const device = new PelsStatusDeviceFixed();
      // Simulate old device that only has pels_status capability
      await device.addCapability('pels_status');
  ...
  describe('updateMode', () => {
    it('should set pels_status capability', async () => {
      const device = new PelsStatusDevice();
      await device.addCapability('pels_status');
      await device.addCapability('alarm_generic');
      ...
      expect(device.getCapabilityValue('pels_status')).toBe('away');
    });
    ...
    it('should not set empty mode', async () => {
      ...
      expect(device.getCapabilityValue('pels_status')).toBeUndefined();
    });
