...
import { applyPriceOptimizationDelta } from '../../lib/plan/planPriceDelta';
...
describe('applyPriceOptimizationDelta', () => {
  it('stores heat in a cheap hour and coasts in an expensive one while heating', () => {
    expect(applyPriceOptimizationDelta(50, config, PriceLevel.CHEAP, 'heating')).toBe(55);
    expect(applyPriceOptimizationDelta(50, config, PriceLevel.EXPENSIVE, 'heating')).toBe(45);
  });
  ...
  it('inverts both moves while cooling', () => {
    ...
    expect(applyPriceOptimizationDelta(22, config, PriceLevel.CHEAP, 'cooling')).toBe(17);
    expect(applyPriceOptimizationDelta(22, config, PriceLevel.EXPENSIVE, 'cooling')).toBe(27);
  });

  it('leaves the target alone at a normal price level, in either direction', () => {
    expect(applyPriceOptimizationDelta(50, config, PriceLevel.NORMAL, 'heating')).toBe(50);
    expect(applyPriceOptimizationDelta(22, config, PriceLevel.NORMAL, 'cooling')).toBe(22);
  });

  it('leaves the target alone when the configured delta is zero', () => {
    const noShift = { enabled: true, cheapDelta: 0, expensiveDelta: 0 };
    expect(applyPriceOptimizationDelta(22, noShift, PriceLevel.CHEAP, 'cooling')).toBe(22);
    expect(applyPriceOptimizationDelta(22, noShift, PriceLevel.EXPENSIVE, 'cooling')).toBe(22);
  });
  ...
  it('reads the configured numbers as magnitudes, not as signs', () => {
    ...
    expect(applyPriceOptimizationDelta(50, backwards, PriceLevel.CHEAP, 'heating')).toBe(55);
    expect(applyPriceOptimizationDelta(50, backwards, PriceLevel.EXPENSIVE, 'heating')).toBe(45);
    expect(applyPriceOptimizationDelta(22, backwards, PriceLevel.CHEAP, 'cooling')).toBe(17);
    expect(applyPriceOptimizationDelta(22, backwards, PriceLevel.EXPENSIVE, 'cooling')).toBe(27);
  });
});
