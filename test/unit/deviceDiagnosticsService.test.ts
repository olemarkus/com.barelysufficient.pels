...
describe('DeviceDiagnosticsService budget-pressure denial integral', () => {
  ...
  const MIN = 60 * 1000;
  const denied = (overrides: Partial<DeviceDiagnosticsPlanObservation> = {}) => buildObservation({
    expectedPowerKw: 2,
  ...
  it('integrates every observed denied span without waiting for a starvation latch', () => {
    const { service } = createDeps();
    const midnight = closeMs();
    sampleEvery5Min(service, midnight - 60 * MIN, midnight, denied({ countingCause: 'capacity' }));

    const totals = service.getDaySuppressionTotals(DAY_KEY);
    expect(totals?.budgetDeniedMs).toBe(60 * MIN);
    expect(totals?.budgetDeniedKwh).toBeCloseTo(2, 6);
    expect(totals?.budgetDenialObserved).toBe(true);
  });

  it('does not integrate demand when budget pressure is ineligible', () => {
    const { service } = createDeps();
    const midnight = closeMs();
    sampleEvery5Min(service, midnight - 60 * MIN, midnight, denied({ budgetPressureDenied: false }));
  ...
  it('skips only an unobserved gap and keeps already persisted denial', () => {
    const { service } = createDeps();
    const midnight = closeMs();
    sampleEvery5Min(service, midnight - 60 * MIN, midnight - 10 * MIN, denied());
    service.observePlanSample({ nowTs: midnight + 15 * MIN, observations: [denied()] });

    const totals = service.getDaySuppressionTotals(DAY_KEY);
    expect(totals?.budgetDeniedMs).toBe(50 * MIN);
    expect(totals?.budgetDeniedKwh).toBeCloseTo(2 * (50 / 60), 6);
  });
  ...
  it('splits the denied accrual correctly across the 25-hour DST-end day', () => {
    ...
    sampleEvery5Min(service, dstMidnight - 60 * MIN, dstMidnight + 5 * MIN, denied());
