...
describe('toSimulationReasonLine — held/limited reasons read hypothetically in simulation', () => {
  it('flips the generic + thermostat hard-cap / daily / hourly "Limited …" lines', () => {
    ...
    expect(toSimulationReasonLine(PLAN_STATE_HOURLY_BUDGET_EXHAUSTED_STATUS, true))
      .toBe("Would be waiting to resume — this capacity period's budget is spent (simulation)");
  });
  ...
  it('is a no-op outside simulation (every reason stays factual)', () => {
    ...
    // In simulation it DOES get a hypothetical rewrite: the device is only
    // waiting because PELS is holding it, so a card scrolled away from the
    // banner must not assert a hold PELS is not performing.
    expect(toSimulationReasonLine(PLAN_STATE_HELD_FALLBACK_STATUS, true))
      .toBe('Would be held back (simulation)');
