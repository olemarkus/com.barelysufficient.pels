...
describe('BatteryStateProducer', () => {
  ...
  describe('battery-id set (targeted-refresh perf hint, IDs not values)', () => {
    const noopEmit = (): void => undefined;

    it('seeds detected battery ids on a non-empty full refresh', () => {
      ...
      expect(producer.getBatteryDeviceIds()).toEqual(['b1']);
    });

    it('re-derives (prunes) the id set on the next non-empty full refresh', () => {
      const producer = new BatteryStateProducer(noopEmit);
      producer.observe([
        battery('b1', { measure_battery: 60, measure_power: 100 }),
        battery('b2', { measure_battery: 70, measure_power: 200 }),
      ], { fullRefresh: true });
      expect(producer.getBatteryDeviceIds().sort()).toEqual(['b1', 'b2']);
      producer.observe([battery('b1', { measure_battery: 61, measure_power: 110 })], { fullRefresh: true });
      expect(producer.getBatteryDeviceIds()).toEqual(['b1']);
    });

    it('a TARGETED refresh never grows the id set', () => {
      const producer = new BatteryStateProducer(noopEmit);
      producer.observe([battery('b1', { measure_battery: 60, measure_power: 100 })], { fullRefresh: false });
      expect(producer.getBatteryDeviceIds()).toEqual([]);
    });

    it('an EMPTY full read leaves the id set intact (benign — re-read next full refresh)', () => {
      const producer = new BatteryStateProducer(noopEmit);
      producer.observe([battery('b1', { measure_battery: 60, measure_power: 100 })], { fullRefresh: true });
      producer.observe([], { fullRefresh: true });
      expect(producer.getBatteryDeviceIds()).toEqual(['b1']);
    ...
    // FIX A: an OFFLINE battery keeps its id (so targeted refreshes re-poll it and
    // it recovers when back online) even though it emits no event.
    it('keeps an OFFLINE battery id in the set so it is still re-polled', () => {
      const producer = new BatteryStateProducer(noopEmit);
      const offline: HomeyDeviceLike = {
        id: 'b1',
        name: 'b1',
        class: 'battery',
        available: false,
        capabilitiesObj: { measure_battery: { value: 62 }, measure_power: { value: 1200 } } as HomeyDeviceLike['capabilitiesObj'],
      };
      producer.observe([offline], { fullRefresh: true });
      expect(producer.getBatteryDeviceIds()).toEqual(['b1']);
    });
