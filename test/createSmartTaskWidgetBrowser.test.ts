...
describe('create smart task widget hidden-element CSS', () => {
  ...
  // preview lines by setting the `hidden` attribute. Any author-origin
  // `display` rule on such an element would beat the user-agent
  // `[hidden] { display: none }` rule on cascade origin (regardless of
  // specificity), so a hidden step would still render — the picker, compose,
  // preview, and created steps stack on top of each other and the widget reads
  // as frozen / unresponsive to taps. The fix is a single blanket author-origin
  // reset, `[hidden] { display: none !important }`, which re-asserts the UA
  // semantics for EVERY hidden-toggled element at once.
  ...
  const cssPath = resolve(process.cwd(), 'widgets/create_smart_task/public/index.css');
  ...
  test('a blanket [hidden] reset re-asserts display:none for every hidden-toggled element', () => {
    // The single guard that covers every step view, the device list, and any
    // future hidden-toggled element in one place. Must be `!important` to win
    // against author `display` rules on the same element regardless of source
    // order — author-vs-author ties break on specificity/order, and `[hidden]`
    // is lower-specificity than the class rules it must override.
  ...
  test('every hidden-toggled element is kept inert while hidden', () => {
    ...
    // a hidden-toggled selector — like the original step-view bug — fails.
