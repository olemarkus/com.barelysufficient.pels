...
For a compatible water heater, **built-in device control** is on by default, so PELS adjusts the heater directly without you wiring up Homey Flows. If you already run a Flow that sets the heater's power level or turns it on or off, PELS leaves built-in control off and shows a notice on the device so the existing setup keeps working. To switch, turn off only the conflicting action in that Flow, then enable built-in device control under the device's **Setup** section. A Flow that only reads the device, or only adjusts its temperature, does not count as a conflict.
