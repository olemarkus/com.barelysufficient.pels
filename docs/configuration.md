...
| **Whole-home meter** | Shown with the Power meter source. Which meter whole-home power readings come from. **Automatic** starts with a sole readable meter marked **Tracks total home energy consumption** and retains it if more later appear during the current app session. If several are present before one is established, select the correct meter directly. A direct selection does not need that marking. The list shows whole-home meters rather than every power-using device, so an EV charger or smart plug won't appear; a meter you chose earlier stays selectable even if it no longer appears. |
