...
- **Built-in device control** lets PELS adjust a supported device (such as a compatible water heater) directly, without you wiring up Homey Flows, and is on by default for those devices. If PELS detects that one of your own Homey Flows already sets that device's power level or turns it on or off, it leaves built-in device control off and shows a notice on the device so it never fights your Flow — remove that Flow to let PELS take over, or turn the switch on under the device's **Setup** section to override. A Flow that only reads the device, or only adjusts its temperature, is not a conflict.
