...
Most configuration starts in **Settings**. The Settings landing page links to **Setup & recommendations**, **Limits & safety**, **Devices**, **Modes**, **Electricity prices**, **Price-aware devices**, **Multiple meters**, **Simulation mode**, and **Advanced**.
...
- Under **Save as current mode target**, a temperature change made outside PELS updates the active mode target. When price adjustments are active, PELS holds that temperature through the current price level, then resumes price deltas when the level changes. Solar-surplus temperature adjustments stay off under this policy; power limiting remains available.
...
is not enough to start it again. The Flow that starts the next booked hour has
to turn the device on as well.
