...
Homey Energy is also a source of per-device power readings. Some devices report their own power directly; others rely on Homey's estimated usage and the values configured in the device's **Energy** settings. PELS can use the live device readings Homey produces from those values. Configured Energy metadata alone does not start control: PELS waits for a device reading. This requirement also applies to temperature devices used only for mode and price behavior.

If a device's energy data looks off, check Homey's **Energy** settings. PELS uses corrected values when Homey reports them. A temporary gap preserves the last trusted reading and your device settings.
