...
- `lib/plan/**` imports the executor in ONE place: `planEngine.ts` instantiates `PlanExecutor` and is the single seam through which the rest of the planner asks the executor anything. The cruiser now ERRORS on any other plan→executor edge (`no-plan-to-executor`, excepting that file); the drift predicates it used to warn about moved to `lib/executor/executorConvergence.ts`. Remaining residue is one type-only edge (`planService.ts` importing `PlanActuationResult`), which `lib/planContract/` would absorb.
