...
- **Both limits stay in force under "Save as current mode target".** That policy keeps price deltas available and switches off solar temperature adjustments, while power limiting remains configured. A manual temperature change becomes the active mode target and cancels an active price shift through the current price level; price deltas resume when the level changes. If the device's temperature is changed outside PELS *while PELS is limiting its temperature*, that change is not saved as the mode's target — it is treated like any other outside change, and PELS brings the device back to its limit. A change made at any other time — including while PELS has the device turned off — is saved as usual.
