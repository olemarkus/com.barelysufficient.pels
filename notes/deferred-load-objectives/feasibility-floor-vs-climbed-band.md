...
- Producer (`diagnosticsBridge.ts` + `policyHorizon.ts`) plans smart tasks in resolved device-priority order (device id breaks ties). Each task publishes its booked admission-power and useful-energy claims; the next task receives `reservedHeadroomKw = max(0, hardCapKw − grossBackgroundKWh/durationHours − higherPriorityAdmissionPowerKw)` and its hourly useful-energy cap minus non-exempt higher-task kWh. This preserves the physical/useful distinction and prevents two tasks from double-booking the same slot while allowing a lower task to fill genuine residual capacity. `grossBackgroundKWh` comes from `plannedGrossUncontrolledKWh` when available, while `plannedUncontrolledKWh` remains the net daily-budget cap input.
