...
| `plugged_in` | PELS wants charge | Connected, allowed, no current — **holdout unknown** | **Not charging** |
| `plugged_in` | PELS wants none | PELS is holding it back | **Not charging**, with the plan's own reason line |
| `plugged_in_discharging` | any | V2G export | **Discharging** |

Row 5 is the one that has caused trouble. "Waiting for car" was originally
asserted here from PELS's intent alone, which blamed the car for a charger PELS
had left idle; #1973 narrowed it to "PELS commanded a powered step". That is
still an inference *about* the car from PELS's own state — better, but not an
observation, and it is retained deliberately: with no car to ask it is the best
evidence available, and dropping it would lose information #1973 added on
purpose. What the association changes is the *grade* of the claim, not whether
it is made — the same words, promoted from inference to observation, plus the
two contradiction rows the charger alone can never produce.

Car rows — the association turns that inference into evidence. These refine only
the ambiguous cells above:

| Charger state | PELS wants | Car says | Reading | Copy |
|---|---|---|---|---|
| `plugged_in` | charge | *no association* | Best available inference | **Waiting for car** — from PELS's commanded step alone (#1973); see the caveat below |
| `plugged_in` | charge | `plugged_in` / `plugged_in_paused` | The car is declining to draw — observed | **Waiting for car** |
| `plugged_in` | charge | `plugged_in_charging` | Charger sees nothing, car thinks it is charging | **Car and charger disagree** |
| `plugged_in` | charge | `plugged_out` | The car says it is not plugged in at all | **Not charging** + the association is suspect; drop it |
| `plugged_in_paused` | charge | `plugged_in_paused` | Both agree the car paused itself | **Paused by the car** |
| `plugged_in_paused` | charge | `plugged_in` | Charger halted, car merely idle | **Paused** |
...
| not asserting delivery | not charging | Nothing has started yet | **Waiting for car** |
...
1. **Never name a cause PELS cannot observe.** Without an association, a
   plugged-in idle charger is `Not charging` and nothing more.
2. **Signal 1 answers "is PELS holding this back", not signal 2.** A lingering
   `true` would otherwise read as "we are allowing it" when PELS is not.
3. **Signal 3 answers "what is happening", not signal 2.**
4. **Contradictions are reported, not smoothed.** Rows 3 and 4 of the car table
   are states PELS previously could not see; they mean something is wrong, and
   quietly picking one side hides it.
