...
// lib/logging/alsContext.ts
var import_node_async_hooks = require("node:async_hooks");
var als = new import_node_async_hooks.AsyncLocalStorage();

// lib/logging/logger.ts
var rootLogger = (0, import_pino.default)({ level: "silent" });
var moduleLoggerCache = /* @__PURE__ */ new Map();
var childByRoot = /* @__PURE__ */ new WeakMap();
var MAX_LOGGER_CACHE_SIZE = 64;
var cacheGrowthWarningEmitted = false;
var liveChild = (module2) => {
  let perRoot = childByRoot.get(rootLogger);
  if (!perRoot) {
    perRoot = /* @__PURE__ */ new Map();
    childByRoot.set(rootLogger, perRoot);
  }
  let child = perRoot.get(module2);
  if (!child) {
    child = rootLogger.child({ module: module2 });
    perRoot.set(module2, child);
  }
  return child;
};
var getLogger = (module2) => {
  const cached = moduleLoggerCache.get(module2);
  if (cached) return cached;
  const proxy = new Proxy({}, {
    get(_target, prop) {
      const live = liveChild(module2);
  ...
  moduleLoggerCache.set(module2, proxy);
...
var zonedPartsFormatterByTimezone = /* @__PURE__ */ new Map();
var offsetFormatterByTimezone = /* @__PURE__ */ new Map();
var getZonedPartsFormatter = (timeZone) => {
...
function getTimeZoneOffsetMinutes(date, timeZone) {
  ...
  try {
    ...
    return Math.round((utcCandidate - date.getTime()) / 6e4);
  } catch (fallbackError) {
    if (!timeZoneOffsetErrorLogged.has(timeZone)) {
      const primaryMessage = primaryError instanceof Error ? primaryError.message : String(primaryError);
      const fallbackMessage = fallbackError instanceof Error ? fallbackError.message : String(fallbackError);
      console.warn(
        `getTimeZoneOffsetMinutes: failed to compute offset for ${timeZone}: ${primaryMessage}; fallback failed: ${fallbackMessage}`
      );
...
var resolveLocalDateTimeMs = (params) => {
  const candidates = resolveAllLocalDateTimeMs(params);
  return candidates.find((candidateMs) => params.earliestAfterMs === void 0 || candidateMs > params.earliestAfterMs) ?? null;
};
var resolveAllLocalDateTimeMs = (params) => {
  const { dateKey, localTime, timeZone } = params;
  const dateParts = parseDateKey2(dateKey);
...
var resolveLocalDateTimeCandidates = (params) => {
...
var matchesLocalTarget = (candidateMs, target, timeZone) => {
...
var ONE_HOUR_MS3 = 60 * ONE_MINUTE_MS;
